...
## 六、Heartbeat 配置与按 Adapter 触发（已实现）

在 `config.yaml` 的 `system` 下增加 `heartbeat` 即可启用心跳。

### 6.1 全局心跳 + 各 Adapter 入口

- **全局心跳**：按配置间隔触发一次「心跳轮次」；Dispatcher 更新全局状态后遍历 adapter。
- **按 adapter 调用**：对每个已注册 adapter，若未实现 **`HeartbeatHandler`** 或 **`HeartbeatEnabled()` 为 false** 则跳过；否则**查询该 adapter 下的 channel**（仅统计 `target_adapter=该 adapter` 的消息，即投递到该 adapter 的 channel），得到本轮要触发的 channel 列表，再调用 **`OnHeartbeat(ctx, opts)`**，opts 中的 **TriggerChannels** 仅含该 adapter 的 channel，不与其他 adapter 混淆。
- **处理逻辑自定**：每个 adapter 自行决定如何响应；无处理可反馈 `POST /api/heartbeat/feedback` 以延长该 channel 间隔或取消。

效果：心跳统一由全局驱动，各 adapter 按需接入；**channel 按 target_adapter 区分**，消息投递到谁就计入谁的 channel，避免 Cursor/Lark 等 channel 混用。
...
system:
  ...
  state_store:
    enabled: true
    db_path: "workdir/mybot.db"
  heartbeat:
    enabled: true
    interval_sec: 300       # 基准间隔（秒）
    min_interval_sec: 60    # 两次触发至少间隔（秒），0 表示不限制
    content: ""              # 心跳消息的指令内容，空则使用内置默认文案
```

### 6.3 无处理则延长间隔与取消

- 执行端（或 adapter）若判定本轮自主轮次**无需要处理的事项**，可调用 **`POST /api/heartbeat/feedback`**，请求体示例：`{"channel":"xxx","user_id":"","no_work":true,"multiplier":2}`。
- 服务端会将该 channel 的当前间隔乘以 `multiplier`（默认 2），并据此更新 `next_due_at`；间隔有上限（如 7 天）。
- **达到最长间隔后仍反馈无处理**：视为该 channel 可能已无用，**取消该 channel 的心跳**（不再对其发送心跳）；若该 channel 之后**有新用户消息**，会自动恢复心跳。

### 6.4 其他说明

- **消息记录**：每条经 dispatch 的消息（各 adapter 收到的与回复的）均写入 StateStore，方便管理查询。
- **对话日志（公共模块）**：`mybot.AppendExchange(workdir, req, resp)` 将本次 request 与 response 追加到 `workdir/.mybot/conversation.md`；各 adapter 在收到请求并产生回复后调用即可，心跳触发时 agent 直接读该文件获取历史，无需心跳再查 StateStore 或附带 recent_messages。
- 心跳消息：`source_adapter=heartbeat`，`Extra["trigger"]=heartbeat`，`user_id` 固定为 `heartbeat`；不附带历史消息，历史由各工作目录下的 `.mybot/conversation.md` 提供。
- Admin：`GET /api/heartbeat` 可查看全局心跳状态（last_trigger_at、trigger_count、next_due_at、enabled）。
